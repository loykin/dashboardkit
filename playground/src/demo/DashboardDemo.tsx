...
function FilterChips({ engine }: { engine: CoreEngineAPI }) {
  ...
  return (
    <>
      <span style={{ width: 1, height: 18, background: 'var(--dk-border)', display: 'inline-block', margin: '0 4px' }} />
...
function VarDropdown({ engine, name }: { engine: CoreEngineAPI; name: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
      ...
      <select className="ex-select" style={{ width: 'auto', minWidth: 90 }}
        value={String(value)} onChange={(e) => setValue(e.target.value)} disabled={loading}>
...
function TimePicker({ stateStore }: { stateStore: DashboardStateStore }) {
  ...
  return (
    <select className="ex-select" style={{ width: 'auto' }}
        ...
        <div key={key} className="field">
          ...
          {field.type === 'number' && (
            <input className="ex-input" type="number"
              ...
              min={field.min} max={field.max} step={field.step ?? (field.integer ? 1 : undefined)}
  ...
  return (
    <>
      <div className="topbar">
        ...
        <button className="btn" onClick={() => nav(`/dashboard/${dash}/variables`)}>⚙ Variables</button>
...
// ── Route: /dashboard/:dashboardId/panels/:panelId/edit ───────────────────────
...
function QueryEditor({ draft, index, dsRecords, onChange, onRemove, canRemove }: {
  draft: QueryDraft
  index: number
  dsRecords: DsRecord[]
  ...
  return (
    <div style={{ border: '1px solid var(--dk-border)', borderRadius: 'var(--dk-radius)', marginBottom: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderBottom: '1px solid var(--dk-border)', background: '#f9f9f9' }}>
        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }}>#{index + 1}</span>
        <select className="ex-select" style={{ flex: 1 }} value={draft.uid}
          onChange={(e) => {
            const rec = dsRecords.find((r) => r.uid === e.target.value)
            if (rec) onChange({ ...draft, uid: rec.uid, dsType: rec.type })
          }}>
          {dsRecords.map((r) => <option key={r.uid} value={r.uid}>{r.name} ({r.type})</option>)}
        </select>
...
function PanelEditorPage() {
  ...
  const back = useCallback(() => nav(`/dashboard/${dashboardId ?? 'sales'}`), [nav, dashboardId])
  ...
  function initDrafts(panel: typeof initPanel): QueryDraft[] {
    ...
    return panel.dataRequests.map((r) => ({
      id: r.id, uid: r.uid, dsType: r.type,
  ...
  function buildDataRequests() {
    return draftsRef.current.filter((d) => d.uid).map((d) => {
      const rec = dsRecords.find((r) => r.uid === d.uid)
      const base = { id: d.id || 'q', uid: d.uid, type: rec?.type ?? d.dsType }
        ...
        <div className="editor-left">
          ...
          <div className="editor-query">
            <div className="editor-query-header">
              ...
              <button className="btn btn-ghost" style={{ fontSize: 11, padding: '3px 8px' }} onClick={addQuery}>+ Add query</button>
            </div>
            <div className="editor-query-body">
              {drafts.map((draft, i) => (
                <QueryEditor key={draft.id + i} draft={draft} index={i} dsRecords={dsRecords}
...
// ── Route: /dashboard/:dashboardId/variables ──────────────────────────────────
...
function VariablesPage() {
  ...
  return (
    <div className="editor-screen">
      <div className="editor-topbar">
        <button className="btn btn-ghost" onClick={() => nav(`/dashboard/${dashboardId ?? 'sales'}`)}>← Back</button>
        ...
        <div>
          ...
          {variables.map((v) => (
            <div key={v.name} className="var-item" style={{ marginBottom: 8 }}>
              ...
              <button className="btn btn-ghost btn-danger btn-icon" style={{ fontSize: 14 }} onClick={() => {
                if (!cfg) return
                void engine.load({ ...cfg, variables: cfg.variables.filter((vv) => vv.name !== v.name) }, { statePolicy: 'preserve' })
              }}>✕</button>
            </div>
          ))}
        </div>
...
function DatasourceListPage() {
  ...
  return (
    <div className="editor-screen">
      <div className="editor-topbar">
        <button className="btn btn-ghost" onClick={() => nav('/dashboard/sales')}>← Dashboard</button>
      ...
      <div style={{ maxWidth: 720, margin: '32px auto', padding: '0 24px' }}>
        ...
        {dsRecords.length === 0 && <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>No datasources configured.</p>}
        {dsRecords.map((ds) => {
          ...
          return (
            <div key={ds.uid} className="var-item" style={{ marginBottom: 8, cursor: 'pointer' }}
              ...
              <div>
                ...
                <div className="var-item-meta">uid: <code>{ds.uid}</code> · type: {ds.type}{plugin?.name ? ` (${plugin.name})` : ''}</div>
              </div>
              <button className="btn btn-ghost btn-icon"
                onClick={(e) => { e.stopPropagation(); nav(`/datasources/${ds.uid}/edit`) }}>Edit →</button>
            </div>
          )
        })}
      </div>
...
function DatasourceEditPage() {
  ...
  useEffect(() => {
    if (isNew) {
      ...
      if (rec) { setSelectedType(rec.type); setDsName(rec.name); setOptions(rec.options) }
    }
  ...
  async function handleTest() {
    if (!plugin?.connector?.healthCheck) {
      setHealth({ ok: true, message: 'No health check defined for this plugin type' }); return
    }
  ...
  function handleDelete() {
    ...
    saveDsRecords(updated); setDsRecords(updated); nav('/datasources')
  }
  ...
  return (
    <div className="editor-screen">
      <div className="editor-topbar">
        ...
        <span style={{ fontWeight: 600, fontSize: 14 }}>{isNew ? 'Add datasource' : `Edit: ${dsName || uid}`}</span>
        <div style={{ flex: 1 }} />
        <button className="btn" onClick={() => void handleTest()} disabled={testing}>{testing ? 'Testing…' : 'Test connection'}</button>
        <button className="btn btn-primary" onClick={() => void handleSave()} disabled={busy || (isNew && !dsUid.trim())}>
          {busy ? 'Saving…' : 'Save'}
        </button>
      </div>
      <div style={{ maxWidth: 640, margin: '32px auto', padding: '0 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ border: '1px solid var(--dk-border)', borderRadius: 'var(--dk-radius)', padding: 16 }}>
          <div className="drawer-section-title" style={{ marginBottom: 12 }}>Connection</div>
          {isNew && (
            <div className="field" style={{ marginBottom: 12 }}>
              <label className="field-label">Plugin type</label>
              <select className="ex-select" value={selectedType} onChange={(e) => {
          ...
          {isNew && (
            <div className="field" style={{ marginBottom: 12 }}>
              <label className="field-label">UID</label>
              <input className="ex-input" placeholder="e.g. my-sales-db" value={dsUid} onChange={(e) => setDsUid(e.target.value)} />
            </div>
          )}
          <div className="field">
...
// ── DashboardLayout: aborts pending queries on section leave ─────────────────

export { DashboardLayout as DashboardAbortLayout }

// ── Page exports ──────────────────────────────────────────────────────────────

export { DashboardPage, PanelEditorPage, VariablesPage, DatasourceListPage, DatasourceEditPage }

// ── Provider — wraps the entire dashboard section with AppContext + dk-root ───
